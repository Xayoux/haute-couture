...
exploration_seuil_haut_gamme <- function(alpha, seuil_2, wb){
...
# Effectuer l'exploration pour chaque seuil pour un seuil 2 à 75%
df <- 
  seuils |> 
  map(
    \(alpha) exploration_seuil_haut_gamme(alpha, seuil_2 = 0.75, wb_concu), 
    .progress = TRUE
  )
...
part_produit_total_function <- function(num){
  df_part_total <- 
    df[[num]][[2]][[2]] |> 
    mutate(
      seuil = seuils[num],
...
1:7 |> 
  map(part_produit_total_function) |> 
  list_rbind() |> 
...
insertPlot(wb_concu, sheet = "Graphiques", startRow = 28, startCol = 1, width = 8, height = 5)
saveWorkbook(wb_concu, file = here("processed-data", "exploration-seuils.xlsx"), overwrite = TRUE)
